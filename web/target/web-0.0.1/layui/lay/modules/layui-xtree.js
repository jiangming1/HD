		...
		for (var i = 0; i < d.length; i++) {
			...
			if (d[i].data.length > 0)
				_this._domStr += '<i class="layui-icon layui-xtree-icon" data-xtree="'
						+ (_this._isopen ? '1' : '0')
						+ '">'
						+ (_this._isopen ? _this._iconOpen : _this._iconClose)
						+ '</i>';
			else {
...
layuiXtree.prototype.checking = function(xtree_items) {
	var num = 0;
	for (var i = 2; i < xtree_items.length; i++) {
		if (xtree_items[i].children.length > 2) {
			var check = _this.checking(xtree_items[i].children.slice(1));
			if (check) {
				xtree_items[i].children[1].checked = true;
				num++;
			}
		} else {
			if (xtree_items[i].children[1].checked) {
				num++;
			}
		}

	}
	if (num >= xtree_items.length - 2) {
		return true;
	} else {
		return false;
	}
...
layuiXtree.prototype.Rendering = function() {
	...
	//检查选中状态
	var xtree_items = _this._container.children;
	for (var i = 0; i < xtree_items.length; i++) {
		var check = _this.checking(xtree_items[i].children);
		if (check) {
			xtree_items[i].children[1].checked = true;
		}
	...
	_this._form
			.on(
					'checkbox(xtreeck' + _this._containerid + ')',
					function(da) {
						//获取当前点击复选框的容器下面的所有子级容器 
						var xtree_chis = da.elem.parentNode
								.getElementsByClassName('layui-xtree-item');
						...
						for (var i = 0; i < xtree_chis.length; i++) {
							if (!_this.getChildByClassName(xtree_chis[i],
								...
								if (da.elem.checked)
									_this.getChildByClassName(xtree_chis[i],
											'layui-xtree-checkbox')[0].nextSibling.classList
											.add('layui-form-checked');
								else
									_this.getChildByClassName(xtree_chis[i],
											'layui-xtree-checkbox')[0].nextSibling.classList
	...
	if (_this._ckall) {
		_this._form
				.on(
						'checkbox(xtreeckall' + _this._containerid + ')',
						function(data) {
							var xtree_allck = data.elem.parentNode.parentNode
									.getElementsByClassName('layui-form-checkbox');
							for (var i = 0; i < xtree_allck.length; i++) {
								if (xtree_allck[i].getAttribute('class')
										.indexOf('layui-checkbox-disbaled') == -1) {
									if (data.elem.checked) {
										xtree_allck[i].classList
												.add('layui-form-checked');
									} else {
										xtree_allck[i].classList
												.remove('layui-form-checked');
									}
									xtree_allck[i].parentNode
											.getElementsByClassName('layui-xtree-checkbox')[0].checked = data.elem.checked;
								}
							}
							_this._ckallSuccess();
						});
...
layuiXtree.prototype.ParendCheck = function(ckelem) {
	...
	if (xtree_p.getAttribute('class') == 'layui-xtree-item') {
		...
		if (xtree_count >= xtree_all.length) {
			...
			_this.getChildByClassName(xtree_p, 'layui-xtree-checkbox')[0].nextSibling.classList
					.add('layui-form-checked');
		} else {
		...
		this.ParendCheck(_this.getChildByClassName(xtree_p,
...
layuiXtree.prototype.GetParent = function(a) {
	...
	for (var i = 0; i < cks.length; i++) {
		if (cks[i].value == a) {
			if (cks[i].parentNode.parentNode.getAttribute('id') == _this._container
					.getAttribute('id'))
				return null;
